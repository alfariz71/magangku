        ...
        <div className="block md:hidden divide-y divide-slate-100">
          {filteredRecords.length === 0 ? (
            ...
            groupedByDate.map(({ date, records }) => {
              ...
              return (
                <div key={`m-group-${date}`} className="space-y-0">
                  {/* Date Header */}
                  <div
                  ...
                  >
                    ...
                    <div className="flex items-center gap-1.5">
                      {label && (
                        <span className="rounded-full bg-[#2F80ED]/10 px-2 py-0.5 text-[10px] font-bold text-[#2F80ED]">
                          {label}
                        </span>
                      )}
                  ...
                  {/* Cards for each attendance */}
                  <div className="p-3 space-y-3">
                    {records.map((item) => (
                      <div
                        key={`m-card-${item.id}`}
                        className="rounded-xl border border-slate-200/70 bg-white p-3.5 shadow-2xs space-y-3"
                      >
                        {/* Student Name & Status */}
                        <div className="flex items-start justify-between gap-2">
                          ...
                          <div className="shrink-0 flex flex-col items-end">
                            ...
                            {item.correctedByAdmin && (
                              <span className="text-[9px] font-semibold text-[#2F80ED] mt-0.5">Dikoreksi Admin</span>
                            )}
                          </div>
                        </div>

                        {/* 3 Columns Time Stats */}
                        <div className="grid grid-cols-3 gap-2 bg-slate-50/80 rounded-lg p-2 text-center text-xs">
                          <div>
                            <span className="block text-[10px] text-slate-400 font-medium mb-0.5">Masuk</span>
                            <span className="font-semibold text-slate-800">{item.checkInTime || '—'}</span>
                          </div>
                          <div className="border-x border-slate-200/60">
                            <span className="block text-[10px] text-slate-400 font-medium mb-0.5">Pulang</span>
                            <span className="font-semibold text-slate-800">{item.checkOutTime || '—'}</span>
                          </div>
                          <div>
                            <span className="block text-[10px] text-slate-400 font-medium mb-0.5">Total</span>
                            <span className="font-semibold text-slate-800">{item.totalHours || '—'}</span>
                          </div>
                        </div>
                        ...
                        <div className="grid grid-cols-2 gap-2 pt-1 border-t border-slate-100">
                          <button
                            onClick={() => openCorrectionModal(item)}
                            className="flex items-center justify-center gap-1.5 rounded-lg border border-slate-200 bg-white py-2 text-xs font-medium text-slate-700 hover:bg-slate-50 hover:border-[#2F80ED] transition"
                          >
                            <Edit3 className="h-3.5 w-3.5 text-[#2F80ED]" />
                            <span>Koreksi</span>
                          </button>
                          <button
                            onClick={() => setDeletingRecord(item)}
                            className="flex items-center justify-center gap-1.5 rounded-lg border border-slate-200 bg-white py-2 text-xs font-medium text-rose-600 hover:bg-rose-50 hover:border-rose-300 dark:bg-slate-800 dark:border-slate-700 dark:text-rose-400 transition"
                          >
                            <Trash2 className="h-3.5 w-3.5 text-rose-500 dark:text-rose-400" />
                            <span>Hapus</span>
                          </button>
